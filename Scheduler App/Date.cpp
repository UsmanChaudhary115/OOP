...
String Date::getDateInFormat1()const
{
	...
	date.concatEqual(toString(month));
	date.concatEqual("/");
	if (day < 10)
	{
		date.concatEqual("0");
	}
	date.concatEqual(toString(day));
	date.concatEqual("/");
	date.concatEqual(toString(year));
	return date;
}
...
String Date::getDateInFormat3()const
{
	...
	for (int i = 0; i < 3; i = i + 1)
	{
		date.concatEqual(monthNames[month - 1].at(i));
	}
	date.concatEqual("-");
	date.concatEqual(toString(year));
	return date;
